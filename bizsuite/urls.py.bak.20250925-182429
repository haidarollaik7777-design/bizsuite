...
def _resolve_model_and_fields():
    ...
    # if model lacks a direct "date" but has entry.date, use entry__date
    try:
...
def report_general_ledger(request):
    """
    GL that works whether the model has a .date field or it's on a related entry (.entry.date).
    Prefers ledger.JournalLine; falls back to settings.REPORTS_POSTING.
    """
    # Prefer JournalLine
    Model = None
    try:
    ...
    # Fallback to configured posting model
    if Model is None:
    ...
    # dates: try date then entry__date
    ...
    if df or dt:
        try:
            if df:
                qs_base = qs_base.filter(date__gte=df)
            if dt:
                qs_base = qs_base.filter(date__lte=dt)
            tried_direct = True
        except Exception:
            tried_direct = False
    if (df or dt) and not tried_direct:
        try:
            if df:
                qs_base = qs_base.filter(entry__date__gte=df)
            if dt:
                qs_base = qs_base.filter(entry__date__lte=dt)
        except Exception:
            pass

    # order
    try:
        qs = qs_base.order_by("date", "id")
    except Exception:
        try:
            qs = qs_base.order_by("entry__date", "id")
        except Exception:
            qs = qs_base.order_by("id")
    ...
    if (request.GET.get("format") or "").lower() == "csv":
        ...
        w = csv.writer(resp)
        w.writerow(["date", "code", "name", "debit", "credit", "memo"])
        for r in rows:
            w.writerow([r["date"], r["code"], r["name"], r["debit"], r["credit"], r["memo"]])
        return resp
...
def report_trial_balance(request):
    ...
    dt = request.GET.get("date_to")
    try:
        if date_lookup and df:
            qs = qs.filter(**{f"{date_lookup}__gte": df})
        if date_lookup and dt:
            qs = qs.filter(**{f"{date_lookup}__lte": dt})
    except Exception:
        pass
...
def report_balance_sheet(request):
    ...
    for a in ann:
        ...
        dr = a["dr"] or Decimal("0")
        cr = a["cr"] or Decimal("0")
        if head in ("4", "7"):  # revenue
            rev_total += (cr - dr)
        elif head in ("5", "6", "8", "9"):  # expenses
            exp_total += (dr - cr)
    ...
    if (request.GET.get("format") or "").lower() == "csv":
        ...
        for r in assets:
            w.writerow(["Assets", r["code"], r["name"], r["balance"]])
        w.writerow(["Totals", "", "Total Assets", _fmt(tot_assets)])
        for r in liab:
            w.writerow(["Liabilities", r["code"], r["name"], r["balance"]])
        for r in equity:
            w.writerow(["Equity", r["code"], r["name"], r["balance"]])
    ...
    return render(request, "admin/accounting/reports/balance_sheet.html", {
        **admin.site.each_context(request),
        ...
        "liabilities": liab,
        "equity": equity,
        "tot_assets": _fmt(tot_assets),
        "tot_liab": _fmt(tot_liab),
        "tot_equity": _fmt(tot_equity),
        "tot_liab_equity": _fmt(tot_liab_equity),
    })
...
def report_income_statement(request):
    ...
    for a in ann:
        ...
        dr = a["dr"] or Decimal("0")
        cr = a["cr"] or Decimal("0")
        if head in ("4", "7"):  # revenue (credit nature)
            amt = cr - dr
            if amt:
                revenue.append({"code": code, "name": name, "amount": _fmt(amt)})
                tot_rev += amt
        elif head in ("5", "6", "8", "9"):  # expenses (debit nature)
            amt = dr - cr
            if amt:
    ...
    if (request.GET.get("format") or "").lower() == "csv":
        ...
        w.writerow(["section", "code", "name", "amount"])
        for r in revenue:
            w.writerow(["Revenue", r["code"], r["name"], r["amount"]])
        w.writerow(["Totals", "", "Total Revenue", _fmt(tot_rev)])
        for r in expense:
            w.writerow(["Expenses", r["code"], r["name"], r["amount"]])
    ...
    return render(request, "admin/accounting/reports/income_statement.html", {
        **admin.site.each_context(request),
        ...
        "revenue": revenue,
        "expense": expense,
        "tot_rev": _fmt(tot_rev),
        "tot_exp": _fmt(tot_exp),
        "net_income": _fmt(net),
    })
...
urlpatterns = [
    ...
    # keep Report Center slug, but redirect it to explicit routes
